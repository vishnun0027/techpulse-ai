...
def test_assign_theme_generative_ai():
    article = {"title": "New LLM breaks all benchmarks", "summary": "A new transformer model..."}
    assert assign_theme(article) == "🧠 Generative AI"


def test_assign_theme_security():
    article = {"title": "Critical CVE found in OpenSSL", "summary": "A new vulnerability was patched..."}
...
def test_assign_theme_developer_tools():
    article = {"title": "New GitHub SDK released", "summary": "The open source API got a major update..."}
    assert assign_theme(article) == "🔧 Developer Tools"


def test_group_by_themes_groups_correctly():
    articles = [
        {"title": "LLM research paper", "summary": "A new llm model study.", "topics": ["🧠 Generative AI", "llm"]},
        {"title": "Github API update",   "summary": "New github sdk release.", "topics": ["🔧 Developer Tools", "sdk"]},
